...
**caption** | **string** | Document caption (may also be used when resending documents by *file\\_id*), 0-1024 characters after entities parsing | [optional] [default to undefined]
**parse_mode** | **string** | Mode for parsing entities in the document caption. See [formatting options](https://core.telegram.org/bots/api/#formatting-options) for more details. | [optional] [default to undefined]
...
**disable_content_type_detection** | **boolean** | Disables automatic server-side content type detection for files uploaded using multipart/form-data | [optional] [default to undefined]
...
```typescript
import { SendDocumentRequest } from 'tele_rest';

const instance: SendDocumentRequest = {
    chat_id,
    document,
